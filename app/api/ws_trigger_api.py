...
async def _message_receiver(conn: WSConnection):
    ...
    while True:
        ...
        if not action:
            await _send_error(conn, "缺少 action 字段", error_code="MISSING_FIELD")
            continue
        ...
        if action == "tool_result":
            ...
            await _send_error(conn, f"未知指令: {action}", error_code="UNKNOWN_ACTION")
...
async def _parse_command(
    ...
    try:
        ...
        await _send_error(conn, f"参数校验失败: {details}", error_code="INVALID_PARAMS")
        return None
...
async def _handle_execute(conn: WSConnection, cmd: WsExecuteCommand):
    ...
    try:
        ...
        await _send_error(conn, str(e), error_code="INTERNAL_ERROR")
    finally:
...
async def _handle_resume(conn: WSConnection, cmd: WsResumeCommand):
    ...
    if target_id is None:
        await _send_error(conn, f"缺少 {key} 字段", error_code="MISSING_FIELD")
        return
...
async def _handle_cancel(conn: WSConnection, cmd: WsCancelCommand):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not gateway:
            await _send_error(conn, "网关不存在", error_code="GATEWAY_NOT_FOUND")
            return
...
async def _handle_delete_session(conn: WSConnection, cmd: WsDeleteSessionCommand):
    ...
    if not success:
        await _send_error(conn, msg, error_code="INTERNAL_ERROR")
        return
...
async def _send_error(
    ...
    **extra,
    ...
    error_code 机器可读枚举：
      - SESSION_INVALID     会话失效/被删/不属于该网关/Agent（客户端应重建会话）
      - EXECUTION_INVALID   执行记录失效/不可取消
      - GATEWAY_NOT_FOUND   网关不存在
      - NOT_AGENT_TYPE      非 Agent 类型不支持该操作
      - SESSION_BUSY        会话正在执行中
      - MISSING_FIELD       缺少必填字段
      - INVALID_PARAMS      参数校验失败
      - INVALID_JSON        消息不是合法 JSON
      - UNKNOWN_ACTION      未知 action 指令
      - IDLE_TIMEOUT        空闲超时（连接即将关闭）
