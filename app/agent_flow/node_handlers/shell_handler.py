...
def _has_multiline(command: str) -> bool:
    ...
    2026-09-13 决定：多行命令在两端均被硬拒绝（Linux sh 原生支持但禁止放开，
    Windows cmd 换行截断问题不再走临时脚本路径）。本函数保留供调用方做入口判定。
    """
    return "\n" in command
...
def _validate_multiline_safety(command: str) -> tuple[bool, str]:
    ...
    validate_command 行 206 first_word 只校验首行首词——多行命令若进入 validate_command
    整段扫描（仅首行首词），中间/末行的危险指令会被漏掉。本函数对每一行单独跑
    validate_command，任意一行命中即拒绝。空行/注释行直接跳过。

    单行输入：等价 validate_command（只跑一次 validate_command）。
    多行输入：每行单独跑，逐行审计。

    注：当前策略是硬拒绝多行（execute_shell 入口），本函数主要为 ssh_handler 等
    其他调用方复用——ssh_handler 先判多行就拒，再用 validate_command 兜底。
    """
...
class ShellNodeHandler(BaseNodeHandler):
    ...
    async def get_tool(self, node: FlowNode) -> Sequence[BaseTool]:
        ...
        async def execute_shell(
            ...
            # 2026-09-13 决定：所有平台硬拒绝多行命令（Linux sh 原生支持多行但禁止放开，
            # Windows cmd 换行截断——统一策略简化语义）。请改写为单行（多条命令
            # 用 && 连接），多行 Python 代码先用 file_write 写入 .py 文件再执行。
            if "\n" in command:
                return {
                    "error": (
                        "命令必须为单行（禁止裸换行）：请将多条独立命令用 && 连成单行；"
                        "多行 Python 代码先用 file_write 写入 .py 文件后以 python <文件路径> 执行。"
                    ),
                    "success": False,
    ...
    async def get_system_prompt_hint(self, node: FlowNode) -> Optional[str]:
        ...
        ps_compat_hint = (
            (
                ...
                "- 命令必须为单行（2026-09-13 决定：所有平台硬拒绝多行）；多条独立命令用 && "
                "连成单行（前一条失败则不继续）；不看成败的顺序执行用单个 &。\n"
