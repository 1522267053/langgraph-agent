...
def _build_mode_prompt(
    ...
    tools：当前 LLM 节点实际可用的工具列表。既用于精准过滤禁用工具列表
    （只有"实际存在且属于禁用前缀"的工具才会被列出），也用于判定「无工具」
    状态：工具集为空时必须转为事实声明，且不再提及任何具体工具名——否则
    模型会按训练分布模仿 tool_call 协议编造工具与执行结果（实测：未连接工具
    节点的流程仍输出 <|minimax|><tool_call><invoke name="python_execution">）。

    工具名相关的硬编码（shell_executor / todowrite）一律按实际工具集条件渲染。
    """
    ...
    has_tools = bool(available_tool_names)
    actually_disabled = [
        name
    ...
    # 工具清单陈述：无工具时给事实声明，有工具时只声明"存在工具"而不枚举——
    # 工具清单已由 API 的 tools 参数下发，在此重复枚举属冗余，且枚举本身会
    # 向模型示范工具名格式（易被套用/仿写成不存在的工具名）。
    # 无工具分支是本次修复核心——原实现只过滤"禁用工具列表"，未声明"工具集为空"
    # 这一状态本身，并硬编码声称 shell_executor / todowrite 可用，导致模型按训练
    # 分布编造工具与执行结果。
    if has_tools:
        inventory = (
            "**工具现状**：你已连接工具，只能调用其中实际提供的工具；"
            "不得调用未提供的工具，也不得输出未实际发生的工具调用格式。"
        )
    else:
        inventory = (
            "**工具现状（事实）**：本节点未连接任何工具节点，你的工具列表为空，"
            "没有任何可用工具。\n"
            "- **禁止**声称调用、正在调用或已调用任何工具；**禁止**描述任何"
            "「已执行 / 已读取 / 已写入 / 已搜索」的结果。\n"
            "- **禁止**输出任何工具调用格式或标签（如 `<tool_call>`、`<invoke>`、"
            "`<function_call>`），也不要模仿此类格式。\n"
            "- 只能基于自身已有知识直接回答；遇到需要工具能力（执行代码、读写文件、"
            "联网搜索等）的请求时，如实告知用户当前未配置工具，并说明可在编排中"
            "连接对应工具节点后再试。"
        )

    if is_plan_mode:
        # 逐条条件渲染：无工具时不给"可以读取文件/搜索代码"，去掉编号空转项，
        # 避免提示词自相矛盾（说了能做实际上做不到）
        clauses = [
            "**只读探索**：可以读取文件、搜索代码、检索知识库来理解问题，"
            "但不得执行写入、编辑、删除或其他修改性操作。"
            if has_tools
            else "**只读约束**：当前没有任何可用工具，只能基于自身已有知识完成分析；"
            "不得声称已读取文件、已搜索代码或已检索知识库。",
        ]
        if disabled_tools:
            clauses.append(
                f"**禁用工具**：以下工具在当前模式不可用：{disabled_tools}。"
                "严禁调用这些工具，也不要尝试用其他工具（如 Shell 重定向）绕过。"
            )
        if "shell_executor" in available_tool_names:
            clauses.append(
                "**Shell 限制**：`shell_executor` 当前可用，但仅允许执行只读、"
                "安全的探索命令；禁止通过 Shell 修改、删除文件或执行破坏性命令。"
            )
        clauses.append(
            "**主动澄清**：如果需求有歧义、边界不清或存在多种方案，先向用户提问确认，"
            "不要对用户意图做大幅假设。"
        )
        clauses.append(
            "**产出计划**：分析完成后，必须产出清晰、可执行的实施计划，"
            "并用 `todowrite` 工具拆解为有序任务。计划应包含：要改动的文件/模块、"
            "具体动作、潜在风险与注意事项。"
            if "todowrite" in available_tool_names
            else "**产出计划**：分析完成后，必须产出清晰、可执行的实施计划，"
            "用 Markdown 列表输出。计划应包含：要改动的文件/模块、具体动作、"
            "潜在风险与注意事项。"
        )
        clauses.append(
            "**回合终止约束**：每轮回复只能以两种方式结束——向用户提出澄清问题，"
            "或输出完整实施计划。不要在未产出计划且未提问的情况下草率结束。"
        )
        numbered = "\n".join(
            f"{index}. {clause}" for index, clause in enumerate(clauses, 1)
        )
        return f"""
...
"""
    ...
    sections = [
        "# 当前运行模式：执行模式（Build Mode）",
        "",
        "你现在处于「执行模式」，可以根据用户需求执行任务。",
        "",
        inventory,
    ]
    if has_tools:
        sections += [
            "",
            "- 如果此前处于计划模式，现已切换到执行模式：此前的只读限制不再适用，"
            "可以正常修改文件、执行命令。",
        ]
        if disabled_tools:
            sections.append(
                f"- 以下工具在计划模式下会被禁用，但在当前执行模式下可用："
                f"{disabled_tools}，请根据任务需要正常使用，并遵守各工具自身的安全限制。"
            )
    return "\n".join(sections)
