...
class TokenStatisticsQuery(BaseModel):
    ...
    time_grain: str = Field(default="day", description="聚合粒度：day/week/month")


class TokenOverview(BaseModel):
    """Token 概览"""

    total_prompt_tokens: int = Field(default=0, description="总输入token数")
    total_completion_tokens: int = Field(default=0, description="总输出token数")
    total_tokens: int = Field(default=0, description="总token数")
    llm_call_count: int = Field(default=0, description="LLM调用次数")


class TokenTrendItem(BaseModel):
    """时间趋势项"""

    date: str = Field(default="", description="日期标签")
...
class TokenByFlowItem(BaseModel):
    ...
    flow_id: int = Field(default=0, description="流程ID")
    flow_name: str = Field(default="", description="流程名称")
    flow_type: str = Field(default="", description="类型：flow/agent")
    prompt_tokens: int = Field(default=0, description="输入token数")
    completion_tokens: int = Field(default=0, description="输出token数")
    total_tokens: int = Field(default=0, description="总token数")
    call_count: int = Field(default=0, description="LLM调用次数")
