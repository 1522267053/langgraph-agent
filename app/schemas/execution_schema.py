...
class FlowExecutionBase(BaseView):
    ...
    flow_name: Optional[str] = Field(default=None, description="流程名称")
    status: Optional[int] = Field(
        default=None, description="状态：0=待执行，1=执行中，2=成功，3=失败，4=已取消"
    )
    input_data: Optional[dict] = Field(default=None, description="输入数据(JSON)")
    output_data: Optional[dict] = Field(default=None, description="输出数据(JSON)")
...
class NodeExecutionBase(BaseView):
    ...
    flow_execution_id: Optional[int] = Field(default=None, description="流程执行记录ID")
    ...
    status: Optional[int] = Field(
        default=None,
        description="状态：0=待执行，1=执行中，2=成功，3=失败，4=跳过，5=已取消",
    )
    input_data: Optional[dict] = Field(default=None, description="输入数据(JSON)")
    output_data: Optional[dict] = Field(default=None, description="输出数据(JSON)")
    error_message: Optional[str] = Field(default=None, description="错误信息")
    ...
    execution_steps: Optional[List[dict]] = Field(
        default=None, description="执行步骤记录(JSON)"
    )
