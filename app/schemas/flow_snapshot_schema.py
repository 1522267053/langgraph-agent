...
class FlowSnapshotBase(BaseView):
    ...
    snapshot_name: Optional[str] = Field(default=None, description="快照名称")
    snapshot_description: Optional[str] = Field(default=None, description="快照描述")
    snapshot_type: Optional[str] = Field(default=None, description="快照类型")
    is_pinned: Optional[int] = Field(default=None, description="是否置顶")


class FlowSnapshotCreate(BaseView):
    """手动创建快照请求"""
