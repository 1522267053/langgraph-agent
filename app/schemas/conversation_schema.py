...
class ConversationMessageBase(BaseView):
    ...
    node_key: Optional[str] = Field(default=None, description="节点标识")
    role: Optional[str] = Field(
        default=None, description="消息角色：system/human/ai/tool"
    )
    content: Optional[str] = Field(default=None, description="消息内容")
    tool_calls: Optional[list[dict[str, Any]]] = Field(
        default=None, description="工具调用请求"
    )
    ...
    sequence: Optional[int] = Field(default=0, description="消息序号")
...
class HumanInputRequest(BaseView):
    ...
    execution_id: Optional[int] = Field(default=None, description="执行记录ID")
    prompt: Optional[str] = Field(default=None, description="提示信息")
    input_variables: Optional[list[dict[str, Any]]] = Field(
        default=None, description="输入变量"
    )
    output_variable: Optional[str] = Field(
        default="human_feedback", description="输出变量名"
    )
    timeout: Optional[int] = Field(default=600, description="超时秒数")
