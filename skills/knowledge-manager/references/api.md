...
| `2` | COMPLETED | 已完成 | 解析分段与向量化完成，可直接语义检索 |
| `3` | FAILED | 失败 | 处理出错，`error_message` 字段含原因 |
| `4` | VECTORIZING | 向量化中 | 正在生成分段向量写入 ChromaDB；向量模型未配置时停留在此，配置后由定时任务续跑 |

**状态流转**：`0 → 1（解析分段）→ 4（向量化）→ 2`；失败时 `→ 3`。
定时任务每 60 秒批量拉取待处理文档，自动完成解析、分段、向量化，无需手动干预。
